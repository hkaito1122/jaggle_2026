"""93_tabpfn_top20pct_on_77

`86_tabpfn_on_84_top30pct`で、84_のTOP30PCT_nested(447列中134列)にTabPFN v2を適用した
ところ、このプロジェクトで初めてTabPFNが単体でCatBoostを上回った
(局所OOF: CB134=0.515581→TabPFN134=0.502742、Public: CB134=0.509500→TabPFN134=0.507300)。
CB134×0.2+TabPFN134×0.8ブレンドはPublic 0.504235まで改善し、現最良アンサンブルにも
15%追加するだけでPublic 0.496207(新記録)に寄与した。

TabPFNは列数が少ないほど強い(`63_tabpfn_ensemble`: 441列→150列で単体val 0.0145改善)
という知見から、84_のTOP30PCT(134列)よりさらに絞ったTOP20PCT(89列)ではTabPFNがどう
振る舞うかを検証する。CatBoostは89列だと悪化することが確認済み(77_基準+0.0016悪化、
[[validation-asymmetry]]参照)だが、TabPFNは列数選好が異なる可能性がある。

63_で確立したTabPFN統合パターンをそのまま踏襲する:
  - `tabpfn==2.2.1`固定(最新8.x系はライセンスゲートに掛かる)
  - GPU必須(CPUだとサンプル数制限)
  - `to_tabpfn_matrix`: NaNは埋めない、カテゴリは序数コード化してcategorical_features_indicesで渡す
  - シード平均([42, 2024, 7])

列選択(CatBoostの重要度上位20%)は各foldの学習データだけでやり直す
(選択リークを避ける、[[validation-asymmetry]]のHow to apply参照)。選択した89列に
対してCatBoostとTabPFNの両方をfitし、リークなしKFold OOFで両者を比較・ブレンド曲線を
確認する([[blend-curve-beats-val-margin-gate]])。

出力: cb89_classifier / tabpfn89_classifier / blend_argmin
...
SCRIPT_NAME = "93_tabpfn_top20pct_on_77"
...
# ============================================================
# TabPFN v2 統合（63_tabpfn_ensembleと同一パターン）
# ============================================================
logger.info("=" * 60)
...
# ============================================================
# リークなしKFold OOF: 各foldでCatBoost選択→CatBoost89/TabPFN89を両方評価
# ============================================================
logger.info("=" * 60)
logger.info("[CB89/TabPFN89] ネストされた特徴量選択+CatBoost/TabPFN比較のOOFを構築中...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)
cb89_oof = np.zeros(len(train_ids))
tabpfn89_oof = np.zeros(len(train_ids))
ncols_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    m_cb89 = _fit_one_classifier(X_tr89, y_tr_cls, obj_top20, OOF_SEED)
    cb89_oof[va_pos] = m_cb89.predict_proba(X_va89)[:, 1]

    # TabPFN(89列、NaN埋めなし、カテゴリは序数コード化)
    ...
    tabpfn89_oof[va_pos] = tp_va

    logger.info(f"    [fold{fold_i}] 完了 (列数={len(top20_cols)})")

cb89_score = log_loss(y_train.values[surv_mask], cb89_oof[surv_mask])
tabpfn89_score = log_loss(y_train.values[surv_mask], tabpfn89_oof[surv_mask])
corr_local = float(np.corrcoef(cb89_oof, tabpfn89_oof)[0, 1])
logger.info(f"[CB89] val(OOF, リークなし, n={surv_mask.sum()})={cb89_score:.6f} (fold毎列数: {ncols_per_fold})")
logger.info(f"[TabPFN89] val(OOF, リークなし, n={surv_mask.sum()})={tabpfn89_score:.6f}")
logger.info(f"[CB89とTabPFN89の相関(OOF)] = {corr_local:.4f}")

# ============================================================
# ブレンド曲線(argmin)を局所OOFで確認 -- ラベル付きデータが両モデルとも揃っているので
# xxxx_v4×プールの時と違い、argmin重みをそのまま採用してよい
# ============================================================
logger.info("=" * 60)
logger.info("[ブレンド曲線] w=CatBoost側の重みを0〜1で走査...")
y_surv = y_train.values[surv_mask]
cb_s = cb89_oof[surv_mask]
tp_s = tabpfn89_oof[surv_mask]
scan_rows = []
...
logger.info(f"[argmin] best_w_cb={best_w:.2f}, best_val={best_ll:.6f} "
            f"(CB89単体比 {best_ll - cb89_score:+.6f} / TabPFN89単体比 {best_ll - tabpfn89_score:+.6f})")

# ============================================================
# OOFの保存(後日プール/xxxx_v4とのブレンドに再利用するため)
# ============================================================
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_cb89_oofpreds.npy", cb89_oof)
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_tabpfn89_oofpreds.npy", tabpfn89_oof)
...
logger.info(f"[提出用] TOP20PCT: {len(top20_cols_full)}列（84_と同一の選択のはず）")
obj_top20_full = [c for c in top20_cols_full if c in obj_cols_full]

X_tr89_full = tf_full[top20_cols_full].fillna(-999)
X_test89_full = ttf_full[top20_cols_full].fillna(-999)

cb_test_preds = []
for seed in SEEDS_SUB:
    m = _fit_one_classifier(X_tr89_full, y_tr_full, obj_top20_full, seed)
    cb_test_preds.append(m.predict_proba(X_test89_full)[:, 1])
    logger.info(f"    [CB89-full] seed={seed}: 完了")
cb89_test = np.mean(cb_test_preds, axis=0)
path1 = save_submission(cb89_test, "cb89_classifier")

logger.info("[TabPFN89-full] シード平均で学習・予測中...")
(tabpfn89_test,) = tabpfn_fit_predict(tf_full, [ttf_full], top20_cols_full, TABPFN_SEEDS)
path2 = save_submission(tabpfn89_test, "tabpfn89_classifier")

blend_test = best_w * cb89_test + (1 - best_w) * tabpfn89_test
path3 = save_submission(blend_test, f"blend_argmin_w{round(best_w * 100):d}pct")

np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_cb89_testpreds.npy", cb89_test)
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_tabpfn89_testpreds.npy", tabpfn89_test)
...
logger.info(f"  {path2}")
logger.info(f"  {path3}")
logger.info(f"(参考: 84_のTOP30PCT_nested Public=0.509500(134列)。86_のTabPFN134 Public=0.507300。"
            f"局所OOF(89列): CB89={cb89_score:.6f} / TabPFN89={tabpfn89_score:.6f} / "

"""83_last_month_category_change_on_77

ベースは`77_last_month_category_on_54`でPublic確認済みの447列
（444 + 最終月の職種/勤務地/部署ID、Public 0.511547、-0.003483改善確定）。

77_は最終月の職種/勤務地/部署IDを「生カテゴリ」として追加したが、初月からの
「変化」自体はまだ特徴量化していない。[[gbdt-interaction-type-matters]]で確認済みの
「クロスフィールド一致/不一致フラグ（L2×M等）はGBDTによく効く」パターンの直接の延長として、
初期職種/初期勤務地/初期部署IDと最終月の職種/勤務地/部署IDを比較した変化フラグ4列を追加する。

追加する4列（すべて0/1または0-3の整数、選択ステップなし・単一の事前登録済み構成）:
  - 職種_初final相違   : 初期職種 != 最終月の職種
  - 勤務地_初final相違 : 初期勤務地 != 最終月の勤務地
  - 部署ID_初final相違 : 初期部署ID != 最終月の部署ID
  - 初final変化数      : 上記3フラグの合計(0〜3)

`81_`で見つかった「特徴量選択のリーク」を繰り返さないため、重要度による絞り込みは一切行わない
（単一構成のみ）。検証は80_/81_/82_と同じリークなしKFold OOF。

出力2ファイル: plus_change_classifier / plus_change_regression_blend
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "83_last_month_category_change_on_77"
...
# ============================================================
# NEW: 初期(persona)カテゴリ vs 最終月カテゴリの「変化」フラグ（4列）
# ============================================================
def create_last_month_category_change_features(persona_df, monthly_df, employee_ids):
    init_job = persona_df.set_index(ID_COL)["初期職種"]
    init_loc = persona_df.set_index(ID_COL)["初期勤務地"]
    init_dept = persona_df.set_index(ID_COL)["初期部署ID"]
    rows = []
    for employee_id in employee_ids:
        emp_data = monthly_df[monthly_df["社員ID"] == employee_id].sort_values("経過月数").reset_index(drop=True)
        last = emp_data.iloc[-1]
        job_changed = int(last["職種"] != init_job.get(employee_id))
        loc_changed = int(last["勤務地"] != init_loc.get(employee_id))
        dept_changed = int(last["部署ID"] != init_dept.get(employee_id))
        rows.append({
            "社員ID": employee_id,
            "職種_初final相違": job_changed,
            "勤務地_初final相違": loc_changed,
            "部署ID_初final相違": dept_changed,
            "初final変化数": job_changed + loc_changed + dept_changed,
        })
...
logger.info("[NEW] 初期カテゴリ vs 最終月カテゴリの変化フラグ(4列)を生成中...")
train_change = create_last_month_category_change_features(train_persona, train_monthly, train_ids)
test_change = create_last_month_category_change_features(test_persona, test_monthly, test_ids)
NEW_BLOCK_COLS = [c for c in train_change.columns if c != ID_COL]
logger.info(f"新規列(初final変化フラグ, 4列): {NEW_BLOCK_COLS}")
for c in NEW_BLOCK_COLS:
    logger.info(f"  {c} 分布(train): {dict(train_change[c].value_counts())}")


# ============================================================
# build_features(train_id_subset): 80_/81_/82_と同一パターン
# ============================================================
def build_features(train_id_subset):
    ...
    tf = tf.merge(train_change, on=ID_COL, how="left")
    ...
    ttf = ttf.merge(test_change, on=ID_COL, how="left")
...
logger.info(f"tf_full 特徴量数: {len(FULL_FEATURE_COLS)}（444+3(77_)+4(初final変化)=451のはず）")
assert len(FULL_FEATURE_COLS) == 451, f"想定外の列数: {len(FULL_FEATURE_COLS)}"

# ============================================================
# 対照群: 77_相当の447列(444+最終月カテゴリ3列、新規4列なし)を同じ5-foldでリークなしOOF
# ============================================================
logger.info("=" * 60)
logger.info(f"[baseline_447] 77_相当(新規4列なし)のリークなしKFold OOFを構築中...")
...
# ============================================================
# 本命: 77_相当447列 + 初final変化4列(451列)のリークなしKFold OOF（選択ステップなし）
# ============================================================
logger.info("=" * 60)
logger.info(f"[plus_change] リークなしKFold OOF(5-fold, seed={OOF_SEED})を構築中(選択ステップなし)...")
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    logger.info(f"    [plus_change] fold{fold_i}: 完了 (特徴量数={len(feat_cols_f)})")

cls_score = log_loss(y_train.values[surv_mask], cls_oof[surv_mask])
logger.info(f"[plus_change] 分類器val(OOF, リークなし, n={surv_mask.sum()})={cls_score:.6f}")
logger.info(f"=== 対照比較: baseline_447(新規4列なし)={base_score:.6f} → plus_change(451列)={cls_score:.6f} "
...
path1 = save_submission(pred_cls, "plus_change_classifier")

# ============================================================
# 回帰ブレンド（77_/81_/82_と同一のOOF-safe設計）
# ============================================================
...
path2 = save_submission(calibrated_test, "plus_change_regression_blend")

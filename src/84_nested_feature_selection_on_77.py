"""84_nested_feature_selection_on_77

`81_`のTOP20PCT/NARROWEDは「重要度上位K%を選ぶ」という特徴量選択ステップをFULL構成の
5-fold CVの最終foldで学習した1モデルの重要度からCVループの外側で1回だけ行い、その固定列
リストをそのままCV評価に使い回していた。これが「特徴量選択のリーク」となり、局所val
(-0.016〜-0.021改善)がPublicで完全反転(+0.0046〜+0.0061悪化)する、プロジェクト最大の
局所val→Public反転を引き起こした([[validation-asymmetry]]2026-08-21追記4)。

本スクリプトは同じ「重要度上位K%を選ぶ」アイデアを、`77_`でPublic確認済みの447列
（新規のノイズ列は一切加えない、純粋な特徴量プルーニングのみ）に対して、選択ステップを
各foldの学習データだけで毎回やり直す**正しくネストされた**形で再検証する
（[[validation-asymmetry]]の"How to apply"ルールに従う）。

構成（すべて単一の事前登録済み構成、追加の探索なし）:
  - baseline_447        : 447列そのまま（比較対象、選択なし）
  - TOP20PCT_nested      : 各foldで学習データのみから重要度上位20%を選び直す
  - TOP30PCT_nested      : 各foldで学習データのみから重要度上位30%を選び直す
    （TOP20PCTが悪化したため、絞りすぎの可能性を確認する追加検証）
  - NARROWED_nested      : 各foldで学習データのみから重要度>0の列を選び直す

R6_lean（113列/441列、列数24.4倍削減、Public確定-0.00093）という「削減が本当に効く」
既存の確認例と同じ削減比率(447列の20%=約89列、22.4倍)を狙う。

提出用（Test予測）の特徴量選択は、Train全件で学習した1モデルの重要度を使う
（Testはラベルを持たないためリークにならない。リークが起きるのはCV評価側だけ）。

出力3ファイル: top20pct_nested_classifier / top30pct_nested_classifier / narrowed_nested_classifier
（回帰ブレンドは54_系パイプラインで4回連続不採用確定済みのため対象外）
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "84_nested_feature_selection_on_77"
...
# ============================================================
# build_features(train_id_subset): 80_/81_/82_/83_と同一パターン（純77_相当447列のみ）
# ============================================================
...
def select_top20pct(imp_series):
    return select_top_pct(imp_series, TOP_PCT)


def select_top30pct(imp_series):
    return select_top_pct(imp_series, TOP_PCT_30)


def select_narrowed(imp_series):
    return imp_series[imp_series > 0].index.tolist()


# ============================================================
# Train全件のtf/ttf（提出用）: baseline_447そのもの
# ============================================================
logger.info("=" * 60)
...
# ============================================================
# baseline_447: 選択なし、リークなしKFold OOF（80_/82_/83_と同一の比較基準）
# ============================================================
logger.info("=" * 60)
logger.info("[baseline_447] 選択なしのリークなしKFold OOFを構築中...")
...
# ============================================================
# TOP20PCT_nested / TOP30PCT_nested / NARROWED_nested: 各foldの学習データだけで選択をやり直す
# ============================================================
logger.info("=" * 60)
logger.info("[TOP20PCT_nested/TOP30PCT_nested/NARROWED_nested] ネストされた特徴量選択でリークなしKFold OOFを構築中...")
top20_oof = np.zeros(len(train_ids))
top30_oof = np.zeros(len(train_ids))
narrowed_oof = np.zeros(len(train_ids))
top20_ncols_per_fold = []
top30_ncols_per_fold = []
narrowed_ncols_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    top30_cols = select_top30pct(imp_series)
    narrowed_cols = select_narrowed(imp_series)
    top20_ncols_per_fold.append(len(top20_cols))
    top30_ncols_per_fold.append(len(top30_cols))
    narrowed_ncols_per_fold.append(len(narrowed_cols))

    obj_top20 = [c for c in top20_cols if c in obj_cols_f]
    obj_top30 = [c for c in top30_cols if c in obj_cols_f]
    obj_narrowed = [c for c in narrowed_cols if c in obj_cols_f]

    m_top20 = _fit_one_classifier(X_tr[top20_cols], y_tr_cls, obj_top20, OOF_SEED)
    top20_oof[va_pos] = m_top20.predict_proba(X_va[top20_cols])[:, 1]

    m_top30 = _fit_one_classifier(X_tr[top30_cols], y_tr_cls, obj_top30, OOF_SEED)
    top30_oof[va_pos] = m_top30.predict_proba(X_va[top30_cols])[:, 1]

    m_narrowed = _fit_one_classifier(X_tr[narrowed_cols], y_tr_cls, obj_narrowed, OOF_SEED)
    narrowed_oof[va_pos] = m_narrowed.predict_proba(X_va[narrowed_cols])[:, 1]

    logger.info(f"    [nested] fold{fold_i}: 完了 (TOP20PCT={len(top20_cols)}列, TOP30PCT={len(top30_cols)}列, "
                f"NARROWED={len(narrowed_cols)}列)")

top20_score = log_loss(y_train.values[surv_mask], top20_oof[surv_mask])
top30_score = log_loss(y_train.values[surv_mask], top30_oof[surv_mask])
narrowed_score = log_loss(y_train.values[surv_mask], narrowed_oof[surv_mask])
logger.info(f"[TOP20PCT_nested] val(OOF, リークなし, n={surv_mask.sum()})={top20_score:.6f} "
            f"(fold毎の列数: {top20_ncols_per_fold})")
logger.info(f"[TOP30PCT_nested] val(OOF, リークなし, n={surv_mask.sum()})={top30_score:.6f} "
            f"(fold毎の列数: {top30_ncols_per_fold})")
logger.info(f"[NARROWED_nested] val(OOF, リークなし, n={surv_mask.sum()})={narrowed_score:.6f} "
            f"(fold毎の列数: {narrowed_ncols_per_fold})")
logger.info(f"=== 対照比較: baseline_447={base_score:.6f} / TOP20PCT_nested={top20_score:.6f} "
            f"(差={top20_score - base_score:+.6f}) / TOP30PCT_nested={top30_score:.6f} "
            f"(差={top30_score - base_score:+.6f}) / NARROWED_nested={narrowed_score:.6f} "
            f"(差={narrowed_score - base_score:+.6f}) ===")
logger.info("(参考: 81_の非ネストTOP20PCT val=0.504990(-0.016改善)だったがPublicで+0.006118悪化に反転。"
            "今回はfold毎に選択をやり直しているため、その反転は原理的に起きないはず。)")
...
top30_cols_full = select_top30pct(imp_series_full)
narrowed_cols_full = select_narrowed(imp_series_full)
logger.info(f"[提出用] TOP20PCT: {len(top20_cols_full)}列 / TOP30PCT: {len(top30_cols_full)}列 / "
            f"NARROWED: {len(narrowed_cols_full)}列")
...
obj_narrowed_full = [c for c in narrowed_cols_full if c in obj_cols_full]

top20_test_preds = []
for seed in SEEDS_SUB:
    m = _fit_one_classifier(X_tr_full[top20_cols_full], y_tr_full, obj_top20_full, seed)
    top20_test_preds.append(m.predict_proba(X_test_full[top20_cols_full])[:, 1])
    logger.info(f"    [TOP20PCT-full] seed={seed}: 完了")
pred_top20 = np.mean(top20_test_preds, axis=0)
path1 = save_submission(pred_top20, "top20pct_nested_classifier")

top30_test_preds = []
for seed in SEEDS_SUB:
    m = _fit_one_classifier(X_tr_full[top30_cols_full], y_tr_full, obj_top30_full, seed)
    top30_test_preds.append(m.predict_proba(X_test_full[top30_cols_full])[:, 1])
    logger.info(f"    [TOP30PCT-full] seed={seed}: 完了")
pred_top30 = np.mean(top30_test_preds, axis=0)
path3 = save_submission(pred_top30, "top30pct_nested_classifier")

narrowed_test_preds = []
for seed in SEEDS_SUB:
    m = _fit_one_classifier(X_tr_full[narrowed_cols_full], y_tr_full, obj_narrowed_full, seed)
    narrowed_test_preds.append(m.predict_proba(X_test_full[narrowed_cols_full])[:, 1])
    logger.info(f"    [NARROWED-full] seed={seed}: 完了")
pred_narrowed = np.mean(narrowed_test_preds, axis=0)
path2 = save_submission(pred_narrowed, "narrowed_nested_classifier")

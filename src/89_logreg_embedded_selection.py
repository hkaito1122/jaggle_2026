"""89_logreg_embedded_selection

`88_logreg_iteration`でロジスティック回帰のCVスコアを少しずつ上げる反復改善を行った
（`85_`のアンサンブルに混ぜられる水準を目指す、[[validation-asymmetry]]の「探索なしの単一
変更」運用を守るため1回ずつ変更して検証）。4イテレーションの結果:

  - iter1 カテゴリ列をOne-Hot→Target Encoding: +0.0007悪化、不採用
  - iter2 正則化をL2→ElasticNet: **-0.008353改善**（0.544021→0.535668、唯一の成功）
  - iter3 列選択をCatBoost重要度→f_classif(線形モデル基準): +0.0026悪化、不採用
  - iter4 数値列に2乗項を追加: +0.0094悪化、不採用

本スクリプトはiter2(One-Hot+ElasticNet)を土台に、5つ目のアイデアとして
**ロジスティック回帰自身の係数でembedded selectionを行う**構成を検証する。
447列全部(One-Hot後約1105次元)でElasticNetを学習し、非ゼロ係数(L1で自動的にゼロ化された
ものを除く)だけに絞って正則化強度を選び直し再学習する——CatBoostの重要度でも
f_classifの周辺関連度でもなく、線形モデル自身が学習した係数を選択基準にする点が
これまでの4イテレーションと異なる。

「全特徴量(LogRegFull)」と「絞り込み後(LogRegSelected)」を同じリークなしKFold OOFで
比較し、CB134(木ベース選択の参照CatBoost)とのブレンド曲線argminも確認したうえで、
Publicに提出して局所valとの乖離を見る。

出力4ファイル: cb134_classifier / logreg_full_classifier / logreg_selected_classifier /
...
SCRIPT_NAME = "89_logreg_embedded_selection"
...
def fit_logreg(X_tr, y_tr):
    # イテレーション2: L2固定 → ElasticNet(L1+L2混合比を探索)に変更。31_のバックログで
    # 「未着手」と明記されていた探索軸。One-Hotで疎になった無関係な列をL1側でゼロ化できるか確認する。
    # 447列(~1105次元)に次元が増えるため、グリッドを少し間引いて速度を確保。
    model = LogisticRegressionCV(
        Cs=np.logspace(-4, 1, 6), l1_ratios=[0.1, 0.5, 0.9],
...
# ============================================================
# ElasticNetの係数(=ロジスティック回帰自身が学んだ重要度)でリファインし、
# 非ゼロ係数の次元だけに絞って再学習する(embedded selection、木ベース/f_classifどちらとも異なる)
# ============================================================
def fit_logreg_with_embedded_selection(X_tr, y_tr, X_va):
    m_full = fit_logreg(X_tr, y_tr)
    pred_full = m_full.predict_proba(X_va)[:, 1]

    coef = m_full.coef_[0]
    mask = np.abs(coef) > 1e-8
    n_selected = int(mask.sum())
    if n_selected < 2:
        # 非ゼロ係数が少なすぎる場合はフル特徴量の結果を流用する
        return m_full, pred_full, m_full, pred_full, X_tr.shape[1]

    m_selected = fit_logreg(X_tr[:, mask], y_tr)
    pred_selected = m_selected.predict_proba(X_va[:, mask])[:, 1]
...
# ============================================================
# リークなしKFold OOF: 447列全部でElasticNetを学習し(選択なし)、
# その係数で非ゼロ次元だけに絞って再学習する(embedded selection)。
# 「全特徴量」と「絞り込み後」を両方OOFで比較する。
# ============================================================
logger.info("=" * 60)
logger.info("[LogRegFull/LogRegSelected/CB134] リークなしKFold OOFを構築中...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)

cb134_oof = np.zeros(len(train_ids))
logreg_full_oof = np.zeros(len(train_ids))
logreg_selected_oof = np.zeros(len(train_ids))
n_selected_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    # LogReg 447列全部(選択なし) → 係数でembedded selection → 再学習
    Xl_tr447, Xl_va447 = prepare_linear_matrices(fold_train_df, [fold_val_df], feat_cols_f)
    _, pred_full, _, pred_selected, n_sel = fit_logreg_with_embedded_selection(
        Xl_tr447, y_tr_cls.values, Xl_va447
    )
    logreg_full_oof[va_pos] = pred_full
    logreg_selected_oof[va_pos] = pred_selected
    n_selected_per_fold.append(n_sel)

    logger.info(f"    [fold{fold_i}] 完了 (LogReg全次元={Xl_tr447.shape[1]}, 非ゼロ係数で絞り込み後={n_sel})")

cb134_score = log_loss(y_train.values[surv_mask], cb134_oof[surv_mask])
logreg_full_score = log_loss(y_train.values[surv_mask], logreg_full_oof[surv_mask])
logreg_selected_score = log_loss(y_train.values[surv_mask], logreg_selected_oof[surv_mask])

logger.info("=" * 60)
logger.info(f"[CB134]           val(OOF, n={surv_mask.sum()}) = {cb134_score:.6f} (参考: 87_/iter2と同一のはず=0.519260)")
logger.info(f"[LogRegFull]      val(OOF, n={surv_mask.sum()}) = {logreg_full_score:.6f}  "
            f"(CB134比 {logreg_full_score - cb134_score:+.6f})")
logger.info(f"[LogRegSelected]  val(OOF, n={surv_mask.sum()}) = {logreg_selected_score:.6f}  "
            f"(CB134比 {logreg_selected_score - cb134_score:+.6f}, "
            f"LogRegFull比 {logreg_selected_score - logreg_full_score:+.6f}, "
            f"fold毎の選択次元数={n_selected_per_fold})")
logger.info("(比較基準: 88_iter2 LogReg134(木ベース選択+ElasticNet) val=0.535668, CB134比+0.016409)")

# ============================================================
# ブレンド曲線(argmin)を局所OOFで確認: CB134 × 良い方のLogReg(Full/Selected)
# ============================================================
logger.info("=" * 60)
y_surv = y_train.values[surv_mask]
cb_s = cb134_oof[surv_mask]
USE_SELECTED = logreg_selected_score <= logreg_full_score
lr_s = logreg_selected_oof[surv_mask] if USE_SELECTED else logreg_full_oof[surv_mask]
logger.info(f"(ブレンド曲線は{'LogRegSelected' if USE_SELECTED else 'LogRegFull'}を採用: "
            f"val {logreg_selected_score:.6f} vs {logreg_full_score:.6f})")
rows = []
...
# ============================================================
# 提出用: Train全件でCB134/LogRegFull/LogRegSelectedを学習してTest予測
...
# LogRegFull(447列全部) → LogRegSelected(非ゼロ係数のみ再学習)
Xl_tr447_full, Xl_test447_full = prepare_linear_matrices(tf_full, [ttf_full], FULL_FEATURE_COLS)
m_lrfull, logregfull_test, m_lrsel, logregsel_test, n_sel_full = fit_logreg_with_embedded_selection(
    Xl_tr447_full, y_tr_full.values, Xl_test447_full
)
logger.info(f"[提出用] LogReg全次元={Xl_tr447_full.shape[1]} → 非ゼロ係数で絞り込み後={n_sel_full}")

lrfull_model_path = SAVED_MODELS_DIR / "logreg_full.joblib"
joblib.dump(m_lrfull, lrfull_model_path)
logger.info(f"  モデル保存: {lrfull_model_path}")
path2 = save_submission(logregfull_test, "logreg_full_classifier")

lrsel_model_path = SAVED_MODELS_DIR / "logreg_selected.joblib"
joblib.dump(m_lrsel, lrsel_model_path)
logger.info(f"  モデル保存: {lrsel_model_path}")
path3 = save_submission(logregsel_test, "logreg_selected_classifier")

lr_test = logregsel_test if USE_SELECTED else logregfull_test
blend_test = best_w * cb134_test + (1 - best_w) * lr_test
...
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_logreg_full_oofpreds.npy", logreg_full_oof)
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_logreg_selected_oofpreds.npy", logreg_selected_oof)

"""129_stacking_with_autogluon89

`122_autogluon_on_89col_pool`(案C: AutoGluonに初めて93_のTOP20PCT_nested89列プールを
投入)の結果、単体OOF val=0.483221(weighted)——115_の8モデルスタックの**どの単体モデル
よりも良く**、スタック全体の入れ子CV(0.482271)にほぼ匹敵する水準だった。102_/104_で
AutoGluonが447列だと弱かった(Public 0.52台)のは「列が多すぎたから」であり、89列に
絞ったら本来の力を発揮したという案Cの仮説が的中したと見られる。既存8モデルとの相関は
0.80〜0.95(cb144/tabpfn144/r_fullとは0.80〜0.86とやや低い)。

115_の8モデルに9つ目として追加、105_/107_/109_/115_/117_/120_と全く同一の設計
（logit変換→L2ロジスティック回帰、外側5-fold(seed=12345)+内側LogisticRegressionCVの
二重入れ子）で再評価する。

判定基準: 分解能±0.0043を正規にクリアした場合のみ採用する（上書き提出は行わない）。

出力: 分解能を超えて改善した場合のみ stacker9_ag89_full_survivor_submission.csv
"""
import datetime
import sys
import warnings
...
SCRIPT_NAME = "129_stacking_with_autogluon89"
...
# ============================================================
# 保存済みOOF/Test予測の読み込み(115_の8モデル + 122_のAutoGluon89列weighted)
# ============================================================
...
BASE_MODELS = {
    ...
    "ag89": ("20260906/20260906_122_autogluon_on_89col_pool_weighted_oofpreds.npy",
             "20260906/20260906_122_autogluon_on_89col_pool_weighted_testpreds.npy"),
}
...
for name in STACK_MODELS:
    if name == "ag89":
        continue
    corr_new[name] = float(np.corrcoef(oof["ag89"][surv_mask], oof[name][surv_mask])[0, 1])
logger.info(f"[相関] ag89 vs 既存8モデル: {corr_new}")

# ============================================================
# スタッキング(入れ子CV) — 9モデルに拡張
# ============================================================
logger.info("=" * 60)
logger.info("logit変換9モデルでのスタッキング(入れ子CV、+ag89)")
...
if improvement_vs_equal > RESOLUTION and improvement_vs_stack8 > RESOLUTION:
    ...
    path = OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_stacker9_ag89_full_survivor_submission.csv"

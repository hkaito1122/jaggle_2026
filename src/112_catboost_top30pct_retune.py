"""112_catboost_top30pct_retune

単層CatBoostの現最良`84_TOP30PCT_nested`（447列中重要度上位30%=134列、Public 0.509500）は
441/444列時代に`38_`等でチューニングされたA_PARAMS（depth=4, learning_rate=0.0352...）を
そのまま流用しており、**134列という縮小された特徴量空間に対して一度も再チューニングされて
いない**。[[hyperparameter-retuning-exhausted]]の2026-08-20追記は「既にチューニング済みの
構成を再探索すると悪化するが、未調整のまま借用していた値を初めてチューニングすると改善する」
という分岐を示しており（`54_`は前者で悪化、`xxxx_v4`は後者で改善）、TOP30PCTのA_PARAMSは
後者に該当する——再挑戦する根拠がある。

**データ数への配慮（ユーザー指摘）**: 「チューニングするほどデータ数は多くない」という懸念に
対し、以下の設計で対応する:
  - Optuna試行数を30回に抑える（旧来の18_〜28_と同じ規模、過剰探索を避ける）
  - 目的関数はリークなしKFold OOF（5-fold）を**3つの異なるfold-shuffle seed(42,2024,7)で
    繰り返し平均**した値にする（`80_`で確立した"repeated_random_kfold_oof"パターン、
    単一splitのノイズに探索が適合するリスクを下げる）
  - 列選択(重要度上位30%)はTOP30PCT_nestedと同じくfoldごとの学習データだけで再計算する
    （選択リークを避ける、[[validation-asymmetry]]のHow to apply）。選択ステップ自体は
    候補ハイパーパラメータに依存しないため、15個(3seed×5fold)の選択結果を**Optuna探索の
    前に1回だけ事前計算**し、各試行では選択済み列に対する最終分類器のfitだけを行う
    （計算量を大幅に削減——選択15回+試行30回×15回fit=465回のCatBoost fit、GPU不要）

反復数は[[hyperparameter-retuning-exhausted]]の「反復数560は113列でも最適だった」という
既存知見を踏襲し固定する（探索対象はdepth/learning_rate/l2_leaf_reg/border_count/
bagging_temperature/random_strengthの6パラメータ、18_〜28_/51_と同一の探索空間）。

判定基準: 新パラメータでの3-seed-repeated OOF valが、同一harnessで測った現行A_PARAMSのval
より分解能±0.0043を超えて改善した場合のみ、Train全件で最終モデルを学習しTest予測を生成する。
（109_/110_の教訓「分解能未満の改善は方向すら信用できない」を踏まえ、この回は自動生成のみ
行い、境界線上の結果は必ずユーザー判断を仰ぐ）

出力: 分解能を超えて改善した場合のみ top30pct_retuned_classifier_submission.csv
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "112_catboost_top30pct_retune"
...
ITER = 560  # [[hyperparameter-retuning-exhausted]]: 113列でも560が最適だった既存知見を踏襲
SEEDS_SUB = [42, 2024, 7, 1234, 99]
TOP_PCT_30 = 0.3
REPEAT_SEEDS = [42, 2024, 7]  # repeated_random_kfold_oof(80_)と同じ3seed
OOF_N_SPLITS = 5
N_TRIALS = 30  # ユーザー指摘(データ数への配慮)を受け、18_〜28_と同規模に抑える
RESOLUTION = 0.0043
...
def _fit_one_classifier(X_tr, y_tr, obj_cols, params, seed):
    model = cb.CatBoostClassifier(**params, iterations=ITER, random_seed=seed,
...
# ============================================================
logger.info("=" * 60)
logger.info("Train全件でのtf/ttfを構築中...")
...
# ============================================================
# 事前計算: 3seed×5fold=15組の「TOP30PCTネスト選択」をOptuna探索の前に1回だけ計算する
# （選択ステップはA_PARAMS固定・候補ハイパーパラメータに依存しないため使い回せる）
# ============================================================
logger.info("=" * 60)
logger.info(f"[事前計算] {len(REPEAT_SEEDS)}seed×{OOF_N_SPLITS}fold=15組のTOP30PCTネスト選択を計算中...")

precomputed = []  # 各要素: dict(va_pos, X_tr, y_tr, X_va, y_va, obj_cols)
for rep_seed in REPEAT_SEEDS:
    skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=rep_seed)
    for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
        ...
        y_tr = tf_f.loc[fold_train_ids, TARGET_COL]

        # 選択ステップ: A_PARAMS固定(38_等の441列時代のチューニング値、選択には触れない)
        m_selector = _fit_one_classifier(X_tr_full, y_tr, obj_cols_f, A_PARAMS, 42)
        ...
        X_tr = tf_f.loc[fold_train_ids, top30_cols].fillna(-999)
        X_va = tf_f.loc[fold_val_ids, top30_cols].fillna(-999)
        y_va = tf_f.loc[fold_val_ids, TARGET_COL] if TARGET_COL in tf_f.columns else None

        precomputed.append(dict(
            rep_seed=rep_seed, fold_i=fold_i, va_pos=va_pos,
            X_tr=X_tr, y_tr=y_tr, X_va=X_va, obj_cols=obj_top30, n_cols=len(top30_cols),
        ))
        logger.info(f"    [seed={rep_seed} fold{fold_i}] 選択完了（列数={len(top30_cols)}）")

logger.info(f"[事前計算完了] {len(precomputed)}組（列数の範囲: "
            f"{min(p['n_cols'] for p in precomputed)}〜{max(p['n_cols'] for p in precomputed)}）")


def evaluate_params(params, seed=42):
    """precomputedの15組に対してparamsで最終分類器をfitし、3seed平均のOOF loglossを返す"""
    per_seed_oof = {s: np.zeros(len(train_ids)) for s in REPEAT_SEEDS}
    for p in precomputed:
        model = _fit_one_classifier(p["X_tr"], p["y_tr"], p["obj_cols"], params, seed)
        per_seed_oof[p["rep_seed"]][p["va_pos"]] = model.predict_proba(p["X_va"])[:, 1]
    seed_scores = [log_loss(y_train.values[surv_mask], per_seed_oof[s][surv_mask]) for s in REPEAT_SEEDS]
    return float(np.mean(seed_scores)), seed_scores


# ============================================================
# ベースライン(現行A_PARAMS)を同一harnessで評価
# ============================================================
logger.info("=" * 60)
logger.info("[ベースライン] 現行A_PARAMSを同一harness(3seed-repeated 5-fold)で評価中...")
baseline_val, baseline_seed_scores = evaluate_params(A_PARAMS)
logger.info(f"[ベースライン] mean_val={baseline_val:.6f} (seed別: {[round(s, 6) for s in baseline_seed_scores]})")

# ============================================================
# Optuna探索(N_TRIALS=30、選択済み15組の最終分類器fitのみを候補ごとに実行)
# ============================================================
logger.info("=" * 60)
logger.info(f"[Optuna探索] N_TRIALS={N_TRIALS}で最終分類器のハイパーパラメータを探索中...")
import optuna
...
best_val, best_seed_scores = evaluate_params(study.best_params)
logger.info(f"[再検証] 再探索パラメータの3seed平均val={best_val:.6f} (seed別: {[round(s, 6) for s in best_seed_scores]})")
logger.info(f"  対現行A_PARAMS: {best_val - baseline_val:+.6f}（分解能±{RESOLUTION}）")

# ============================================================
# 判定: 分解能を超えて改善した場合のみ、Train全件で最終モデルを学習しTest予測を生成
# ============================================================
improvement = baseline_val - best_val
logger.info("=" * 60)
if improvement > RESOLUTION:
    logger.info(f"[判定] 分解能を超えて改善({improvement:+.6f})——Train全件で最終モデルを学習しTest予測を生成する")

    m_selector_full = _fit_one_classifier(
        tf_full[FULL_FEATURE_COLS].fillna(-999), tf_full[TARGET_COL],
        [c for c in FULL_FEATURE_COLS if tf_full[c].dtype == "object"], A_PARAMS, 42,
    )
    ...
    obj_top30_full = [c for c in top30_cols_full if tf_full[c].dtype == "object"]
    logger.info(f"[提出用] TOP30PCT(再チューニング版): {len(top30_cols_full)}列")

    X_tr_full = tf_full[top30_cols_full].fillna(-999)
    y_tr_full = tf_full[TARGET_COL]
    X_test_full = ttf_full[top30_cols_full].fillna(-999)

    test_preds = []
    for seed in SEEDS_SUB:
        m = _fit_one_classifier(X_tr_full, y_tr_full, obj_top30_full, study.best_params, seed)
        test_preds.append(m.predict_proba(X_test_full)[:, 1])
        logger.info(f"    seed={seed}: 完了")
    pred = np.mean(test_preds, axis=0)
    path = save_submission(pred, "top30pct_retuned_classifier")
    logger.info(f"  提出ファイル: {path}")
else:
    logger.info(f"[判定] 分解能未満または悪化({improvement:+.6f})——再チューニングは不採用。"
                f"Test予測は生成しない（境界線上の場合はユーザー判断を仰ぐこと）")

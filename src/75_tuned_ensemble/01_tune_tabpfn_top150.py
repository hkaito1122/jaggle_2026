...
# (data/output/_checkpoints/75_tuned_ensemble_cache_fm/) の top150(CatBoost重要度上位150列)を使い、
...
SCRIPT_NAME = "75_01_tune_tabpfn_top150"
...
FEATS = FEATURE_SETS["top150"]
print(f"top150: {len(FEATS)}列 / 学習{len(ag_train_80b)}名 / 検証{len(ag_val_surv)}名 / 全件{len(ag_full)}名")
...
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_tabpfn_top150_tuned_testpreds.npy", pred_tuned)
sub = pd.DataFrame({ID_COL: test_ids, "定着確率": pred_tuned})
path = OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_tabpfn_top150_tuned_submission.csv"

"""91_shrinkage_lda_on_77

初回実行（木ベース134列選択+Shrinkage LDA、val=0.556857、CB134比+0.037597）は
88_のElasticNet版ロジスティック回帰(0.535668)にも及ばなかった([[cpu-model-zoo-closed]]参照)。
本バージョンは列選択そのものをCatBoostの重要度ではなく、**BayesianRidgeの係数**に置き換える。
447列全部(One-Hot後約1105次元)でBayesianRidgeを学習し、係数(|coef|)の絶対値上位50%の
次元だけをShrinkage LDAの入力にする——選択も学習も両方とも線形モデル(ベイズ的縮小推定+
判別分析)で完結させる構成。CB134(木ベース選択のCatBoost)は比較の参照値としてのみ使う。

`sklearn.discriminant_analysis.LinearDiscriminantAnalysis(solver="lsqr", shrinkage="auto")`
はsvd以外のsolverでのみshrinkageに対応。shrinkage="auto"はLedoit-Wolf基準で
縮小係数を自動推定する。

CB134(木ベース選択の参照CatBoost)とのブレンド曲線(w=0〜1)のargminも局所OOFで確認したうえで、
Publicに提出して局所valとの乖離を見る。

出力3ファイル: cb134_classifier / lda_br50_classifier / blend_argmin_w{N}pct
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "91_shrinkage_lda_on_77"
...
def fit_lda(X_tr, y_tr):
    # 共分散行列をLedoit-Wolf基準で自動縮小推定するShrinkage LDA。
    # svd以外のsolverでのみshrinkageに対応するためlsqrを使う。
    model = LinearDiscriminantAnalysis(solver="lsqr", shrinkage="auto")
    model.fit(X_tr, y_tr)
    return model


TOP_PCT_BR = 0.5  # BayesianRidgeの係数(|coef|)上位50%をLDAの入力に使う


def select_by_bayesridge_coef(X_tr, y_tr, pct):
    """447列全部(One-Hot後の全次元)でBayesianRidgeを学習し、係数の絶対値上位pctの
    次元マスクを返す。CatBoostの重要度ではなく、線形モデル自身の係数を選択基準にする。"""
    br = BayesianRidge(max_iter=500, tol=1e-4)
    br.fit(X_tr, y_tr)
    coef = br.coef_
    n_top = max(1, round(len(coef) * pct))
...
# ============================================================
# リークなしKFold OOF: BayesianRidgeの係数上位50%で選んだ次元に Shrinkage LDA を適用。
# CB134(木ベース選択のCatBoost)は比較の参照値としてのみ使う。
# ============================================================
logger.info("=" * 60)
logger.info("[LDA_BR50/CB134] リークなしKFold OOFを構築中(BayesianRidge係数選択+Shrinkage LDA)...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)

cb134_oof = np.zeros(len(train_ids))
lda134_oof = np.zeros(len(train_ids))
n_selected_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    # BayesianRidgeの係数上位50%で選んだ次元にShrinkage LDAを適用
    Xl_tr447, Xl_va447 = prepare_linear_matrices(fold_train_df, [fold_val_df], feat_cols_f)
    mask = select_by_bayesridge_coef(Xl_tr447, y_tr_cls.values, TOP_PCT_BR)
    n_selected_per_fold.append(int(mask.sum()))
    m_lda134 = fit_lda(Xl_tr447[:, mask], y_tr_cls.values)
    lda134_oof[va_pos] = m_lda134.predict_proba(Xl_va447[:, mask])[:, 1]

    logger.info(f"    [fold{fold_i}] 完了 (全次元={Xl_tr447.shape[1]}, BayesRidge上位50%選択後={int(mask.sum())})")

cb134_score = log_loss(y_train.values[surv_mask], cb134_oof[surv_mask])
lda134_score = log_loss(y_train.values[surv_mask], lda134_oof[surv_mask])

logger.info("=" * 60)
logger.info(f"[CB134]     val(OOF, n={surv_mask.sum()}) = {cb134_score:.6f} (参考: 87_と同一のはず=0.519260)")
logger.info(f"[LDA_BR50]  val(OOF, n={surv_mask.sum()}) = {lda134_score:.6f}  "
            f"(CB134比 {lda134_score - cb134_score:+.6f}, fold毎の選択次元数={n_selected_per_fold})")
logger.info("(比較基準: 91_の木ベース134列選択版LDA val=0.556857, CB134比+0.037597)")
...
cb_s, lr_s = cb134_oof[surv_mask], lda134_oof[surv_mask]
rows = []
...
# ============================================================
# 提出用: Train全件でCB134(参照)とLDA_BR50を学習してTest予測
...
mask_full = select_by_bayesridge_coef(Xl_tr447_full, y_tr_full.values, TOP_PCT_BR)
logger.info(f"[提出用] BayesRidge係数選択: 全次元={Xl_tr447_full.shape[1]} → 上位50%={int(mask_full.sum())}")
m_lda134_full = fit_lda(Xl_tr447_full[:, mask_full], y_tr_full.values)
lda134_test = m_lda134_full.predict_proba(Xl_test447_full[:, mask_full])[:, 1]
lda_model_path = SAVED_MODELS_DIR / "lda_br50.joblib"
joblib.dump(m_lda134_full, lda_model_path)
logger.info(f"  モデル保存: {lda_model_path}")
path2 = save_submission(lda134_test, "lda_br50_classifier")

blend_test = best_w * cb134_test + (1 - best_w) * lda134_test
...
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_lda134_oofpreds.npy", lda134_oof)

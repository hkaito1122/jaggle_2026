"""117_stacking_with_xgboost

`116_`で84_プール(134列)にXGBoost単体を適用した(early stopping込みで最初から設計、
単体val=0.537346、既存8モデルとの相関は未確認)。115_の8モデルに9つ目として追加、
105_/107_/109_/115_と全く同一の設計（logit変換→L2ロジスティック回帰、外側5-fold
(seed=12345)+内側LogisticRegressionCVの二重入れ子）で再評価する。

判定基準は変わらず: 分解能±0.0043を正規にクリアした場合のみ採用する
（`109_`の反転・`115_`の的中という2つの前例を踏まえ、事前登録ルールの上書きは行わない）。

出力: 分解能を超えて改善した場合のみ stacker9_xgb_full_survivor_submission.csv
"""
import datetime
import sys
import warnings
...
SCRIPT_NAME = "117_stacking_with_xgboost"
...
# ============================================================
# 保存済みOOF/Test予測の読み込み(115_の8モデル + 116_のxgb134)
# ============================================================
...
BASE_MODELS = {
    ...
    "xgb134": ("20260905/20260905_116_xgboost_on_84_top30pct_xgb134_oofpreds.npy",
               "20260905/20260905_116_xgboost_on_84_top30pct_xgb134_testpreds.npy"),
}
...
for name in STACK_MODELS:
    if name == "xgb134":
        continue
    corr_xgb[name] = float(np.corrcoef(oof["xgb134"][surv_mask], oof[name][surv_mask])[0, 1])
logger.info(f"[相関] xgb134 vs 既存8モデル: {corr_xgb}")

# ============================================================
# スタッキング(入れ子CV) — 105_/107_/109_/115_と全く同一設計、9モデルに拡張
# ============================================================
logger.info("=" * 60)
...
if improvement_vs_equal > RESOLUTION and improvement_vs_stack8 > RESOLUTION:
    ...
    path = OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_stacker9_xgb_full_survivor_submission.csv"

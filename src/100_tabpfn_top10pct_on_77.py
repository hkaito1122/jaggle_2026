"""100_tabpfn_top10pct_on_77

TabPFN v2はこのプロジェクトで一貫して「列数が少ないほど強い」という単調な傾向を示してきた:

| スクリプト | 列数 | TabPFN単体Public |
|---|---|---|
| `63_`(441列) | 441 | (441列版、CatBoost比+0.0158) |
| `86_`(TOP30PCT_nested) | 134 | 0.507300 |
| `93_`(TOP20PCT_nested) | **89** | **0.504065（更新）** |

`93_`まで一度も反転しておらず、この単調な傾向がまだ続くかを確認するため、さらに絞った
TOP10PCT（447列の10%≒45列）を検証する。ユーザーの指示により「TabPFNの列数削減をさらに追う」
方針で、`88_`と同じ「1回に1パラメータだけ変える」反復スタイルを踏襲し、TOP_PCTを0.20→0.10に
変えるだけの単一の変更にとどめる。

CatBoostは列を削るほど一貫して悪化してきた（`84_`のTOP20PCT/TOP30PCT/NARROWED/FULLが
単調な関係、[[validation-asymmetry]]2026-08-22追記5）が、TabPFNは逆の選好を持つため、
45列という極端な削減でCatBoostがさらに悪化する一方でTabPFNがさらに改善するかを両方観測する。

93_と同じ設計を踏襲: 列選択(CatBoostの重要度上位10%)は各foldの学習データだけでやり直す
（選択リークを避ける、[[validation-asymmetry]]のHow to apply参照）。選択した列に対して
CatBoostとTabPFNの両方をfitし、リークなしKFold OOFで両者を比較・ブレンド曲線を確認する
（[[blend-curve-beats-val-margin-gate]]）。

出力: cb10_classifier / tabpfn10_classifier / blend_argmin
...
SCRIPT_NAME = "100_tabpfn_top10pct_on_77"
...
OOF_SEED = 42
OOF_N_SPLITS = 5
TOP_PCT_10 = 0.10  # 93_のTOP_PCT_20(0.2)から変更する唯一のパラメータ
...
def select_top10pct(imp_series):
    return select_top_pct(imp_series, TOP_PCT_10)
...
# ============================================================
# リークなしKFold OOF: 各foldでCatBoost選択→CatBoost10/TabPFN10を両方評価
# ============================================================
logger.info("=" * 60)
logger.info("[CB10/TabPFN10] ネストされた特徴量選択(上位10%)+CatBoost/TabPFN比較のOOFを構築中...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)
cb10_oof = np.zeros(len(train_ids))
tabpfn10_oof = np.zeros(len(train_ids))
ncols_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    # 選択ステップ: このfoldの学習データだけで重要度を計算(84_/93_と同一パターン)
    ...
    top10_cols = select_top10pct(imp_series)
    ncols_per_fold.append(len(top10_cols))
    obj_top10 = [c for c in top10_cols if c in obj_cols_f]

    # CatBoost(10%列)
    X_tr10 = tf_f.loc[fold_train_ids, top10_cols].fillna(-999)
    X_va10 = tf_f.loc[fold_val_ids, top10_cols].fillna(-999)
    m_cb10 = _fit_one_classifier(X_tr10, y_tr_cls, obj_top10, OOF_SEED)
    cb10_oof[va_pos] = m_cb10.predict_proba(X_va10)[:, 1]

    # TabPFN(10%列、NaN埋めなし、カテゴリは序数コード化)
    fold_train_df = tf_f.loc[fold_train_ids]
    fold_val_df = tf_f.loc[fold_val_ids]
    (tp_va,) = tabpfn_fit_predict(fold_train_df, [fold_val_df], top10_cols, TABPFN_SEEDS)
    tabpfn10_oof[va_pos] = tp_va

    logger.info(f"    [fold{fold_i}] 完了 (列数={len(top10_cols)})")

cb10_score = log_loss(y_train.values[surv_mask], cb10_oof[surv_mask])
tabpfn10_score = log_loss(y_train.values[surv_mask], tabpfn10_oof[surv_mask])
corr_local = float(np.corrcoef(cb10_oof, tabpfn10_oof)[0, 1])
logger.info(f"[CB10] val(OOF, リークなし, n={surv_mask.sum()})={cb10_score:.6f} (fold毎列数: {ncols_per_fold})")
logger.info(f"[TabPFN10] val(OOF, リークなし, n={surv_mask.sum()})={tabpfn10_score:.6f}")
logger.info(f"[CB10とTabPFN10の相関(OOF)] = {corr_local:.4f}")
logger.info(f"(参考: 93_のTOP20PCT(89列) CB89=0.518415/TabPFN89=0.503880。"
            f"単調な傾向が続くなら本結果はTabPFN10<0.503880・CB10>0.518415になるはず)")
...
cb_s = cb10_oof[surv_mask]
tp_s = tabpfn10_oof[surv_mask]
scan_rows = []
...
logger.info(f"[argmin] best_w_cb={best_w:.2f}, best_val={best_ll:.6f} "
            f"(CB10単体比 {best_ll - cb10_score:+.6f} / TabPFN10単体比 {best_ll - tabpfn10_score:+.6f})")

# ============================================================
# OOFの保存(後日プール/xxxx_v4とのブレンドに再利用するため)
# ============================================================
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_cb10_oofpreds.npy", cb10_oof)
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_tabpfn10_oofpreds.npy", tabpfn10_oof)
...
top10_cols_full = select_top10pct(imp_series_full)
logger.info(f"[提出用] TOP10PCT: {len(top10_cols_full)}列")
obj_top10_full = [c for c in top10_cols_full if c in obj_cols_full]

X_tr10_full = tf_full[top10_cols_full].fillna(-999)
X_test10_full = ttf_full[top10_cols_full].fillna(-999)

cb_test_preds = []
for seed in SEEDS_SUB:
    m = _fit_one_classifier(X_tr10_full, y_tr_full, obj_top10_full, seed)
    cb_test_preds.append(m.predict_proba(X_test10_full)[:, 1])
    logger.info(f"    [CB10-full] seed={seed}: 完了")
cb10_test = np.mean(cb_test_preds, axis=0)
path1 = save_submission(cb10_test, "cb10_classifier")

logger.info("[TabPFN10-full] シード平均で学習・予測中...")
(tabpfn10_test,) = tabpfn_fit_predict(tf_full, [ttf_full], top10_cols_full, TABPFN_SEEDS)
path2 = save_submission(tabpfn10_test, "tabpfn10_classifier")

blend_test = best_w * cb10_test + (1 - best_w) * tabpfn10_test
path3 = save_submission(blend_test, f"blend_argmin_w{round(best_w * 100):d}pct")

np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_cb10_testpreds.npy", cb10_test)
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_tabpfn10_testpreds.npy", tabpfn10_test)
...
logger.info(f"  {path2}")
logger.info(f"  {path3}")
logger.info(f"(参考: 84_のTOP30PCT_nested Public=0.509500(134列)。93_のTabPFN89 Public=0.504065。"
            f"局所OOF(10%列): CB10={cb10_score:.6f} / TabPFN10={tabpfn10_score:.6f} / "

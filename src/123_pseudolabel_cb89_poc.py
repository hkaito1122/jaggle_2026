"""123_pseudolabel_cb89_poc

ユーザーから「ダメ元でも良いので劇的変化が期待される案」の提示を求められ、3案
(A: LLM系テキスト特徴量抽出/商用利用不可のため却下、B: 疑似ラベリング、
C: AutoGluonへの89列プール投入=122_)を提示したところ、Bの実施が承認された。

[[monthly-data-information-ceiling]]で「月次データは10年定着をほとんど予測しない」
ことが確定して以来、疑似ラベリング(モデル自身の予測をラベルとして再学習に使う手法)は
「弱いモデルの予測を信じて増幅するだけ」という理由で検討候補にすら上がらなかった。
しかし現在のモデル(115_の8モデルスタッキング、Public 0.493891)は当時と比べて大幅に
強化されており(94_add_w25の0.495570から107_/115_と2段階更新)、高信頼度領域の予測は
以前より信頼できる可能性がある——「状況が変わった」との位置付けで再検討する。

設計方針(スコープを絞った単一モデルでの概念実証):
  - 疑似ラベル源: 115_の現在のPublic最良提出(8モデルスタッキング)のTest予測
    (data/output/20260905/..._115_..._submission.csv)をそのまま使う
  - 対象モデル: 93_のCB89(84_/93_で確立済みの84_TOP20PCT_nested、89列選択CatBoost、
    局所val=0.518415)。TabPFNではなくCatBoostを選ぶ理由は高速・決定的・ローカルCPUで
    完結し概念実証に向くため([[cpu-model-zoo-closed]]の高速性評価と同じ判断基準)
  - 93_と全く同一のネストされた5-fold(seed=42)OOF設計を再現し、各foldで:
      1. fold_train_idsのみでbuild_features()を呼ぶ(93_と同一、選択リーク回避)
      2. CatBoost重要度でTOP20PCT(89列)を選択(93_と同一)
      3. 同じfit_idsで leak-avoidedなttf_f(Test特徴量)を得る(部署ターゲット
         エンコーディングもfold_train_idsのみでfit、93_のbuild_features()の既存動作を
         そのまま利用——新しいリーク経路を作らない)
      4. 疑似ラベル: 115_のTest予測のうち閾値(0.02/0.98, 0.05/0.95, 0.1/0.9, 0.15/0.85の
         4段階をスキャン)を超える高信頼度行のみをhard label化(proba>0.5→1、それ以外0)
         して抽出し、fold学習データに追加(sample_weight=1.0、まずは単純に等重み)
      5. 疑似ラベル追加ありのCB89と、追加なしの素のCB89(93_と同一)を同一fold・同一
         選択で学習し、同一の未疑似ラベル・未学習の実val foldで比較する
         (val foldは一切汚染されない——疑似ラベルはTest由来でTrainのIDと重複しない)

判定基準: [[validation-asymmetry]]で確立した分解能±0.0043を正規にクリアした場合のみ、
「疑似ラベリングは有効」と判断する。閾値グリッドの中で最良のものが分解能を超えて
改善した場合のみ、そのCB89_pseudo版のOOF/Test予測を保存し後日スタッキングに使う
候補とする。分解能未満なら不採用とし、上書き提出は行わない([[ensemble-oof-overfitting]]の
109_の反転事例を踏まえた事前登録ルール)。

出力: 分解能を超えて改善した閾値が見つかった場合のみ
cb89_pseudo_best_oofpreds.npy / cb89_pseudo_best_testpreds.npy を保存
（後日105_/107_/115_と同様のスタッキング系列に組み込むための布石）。
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "123_pseudolabel_cb89_poc"
...
def _fit_one_classifier(X_tr, y_tr, obj_cols, seed, sample_weight=None):
...
# ============================================================
# 93_と同一のネストされた5-fold OOF設計を再現し、各fold・各閾値で
# 「疑似ラベル追加あり」vs「追加なし(素のCB89)」を同一条件で比較する
# ============================================================
logger.info("=" * 60)
logger.info("[CB89 vs CB89+疑似ラベル] ネストされた特徴量選択+閾値スキャンのOOFを構築中...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)
cb89_baseline_oof = np.zeros(len(train_ids))
cb89_pseudo_oof = {t: np.zeros(len(train_ids)) for t in THRESHOLDS}
ncols_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    # 選択ステップ: このfoldの学習データだけで重要度を計算(93_と同一パターン)
    ...
    top20_cols = select_top_pct(imp_series, TOP_PCT_20)
    ncols_per_fold.append(len(top20_cols))
    obj_top20 = [c for c in top20_cols if c in obj_cols_f]

    X_tr89 = tf_f.loc[fold_train_ids, top20_cols].fillna(-999)
    X_va89 = tf_f.loc[fold_val_ids, top20_cols].fillna(-999)

    # ベースライン: 疑似ラベルなしの素のCB89(93_と同一)
    m_baseline = _fit_one_classifier(X_tr89, y_tr_cls, obj_top20, OOF_SEED)
    cb89_baseline_oof[va_pos] = m_baseline.predict_proba(X_va89)[:, 1]

    # 疑似ラベル用Test特徴量(このfoldのfold_train_idsだけでfitしたleak-avoidedなttf_f)
    X_test89 = ttf_f[top20_cols].fillna(-999)

    for t in THRESHOLDS:
        pseudo_mask = (best_sub < t) | (best_sub > 1 - t)
        pseudo_X = X_test89[pseudo_mask]
        pseudo_y = pd.Series((best_sub[pseudo_mask] > 0.5).astype(int), index=pseudo_X.index)
        X_aug = pd.concat([X_tr89, pseudo_X], axis=0)
        y_aug = pd.concat([y_tr_cls, pseudo_y], axis=0)
        m_pseudo = _fit_one_classifier(X_aug, y_aug, obj_top20, OOF_SEED)
        cb89_pseudo_oof[t][va_pos] = m_pseudo.predict_proba(X_va89)[:, 1]

    logger.info(f"    [fold{fold_i}] 完了 (列数={len(top20_cols)}, "
                f"疑似ラベル行数={ {t: int(((best_sub < t) | (best_sub > 1 - t)).sum()) for t in THRESHOLDS} })")

cb89_baseline_score = log_loss(y_train.values[surv_mask], cb89_baseline_oof[surv_mask])
logger.info(f"[CB89ベースライン(疑似ラベルなし)] val(OOF, リークなし)={cb89_baseline_score:.6f} "
            f"(93_の参考値0.518415と一致するはず、fold毎列数: {ncols_per_fold})")

logger.info("=" * 60)
logger.info("[閾値スキャン結果]")
results = {}
for t in THRESHOLDS:
    score = log_loss(y_train.values[surv_mask], cb89_pseudo_oof[t][surv_mask])
    diff = cb89_baseline_score - score
...
if best_diff > RESOLUTION:
    logger.info(f"[判定] 分解能を超えて改善({best_diff:+.6f} > {RESOLUTION})——"
                f"疑似ラベリング(t={best_t:.2f})は有効。全データで最終CB89_pseudoを学習しOOF/Test予測を保存する")
    tf_final, ttf_final = build_features(train_ids.tolist())
    feat_cols_final = _feature_cols(tf_final)
    obj_cols_final = [c for c in feat_cols_final if tf_final[c].dtype == "object"]
    X_final = tf_final[feat_cols_final].fillna(-999)
    y_final = tf_final[TARGET_COL]
    m_sel_final = _fit_one_classifier(X_final, y_final, obj_cols_final, OOF_SEED)
    imp_final = pd.Series(m_sel_final.get_feature_importance(), index=feat_cols_final)
    top20_final = select_top_pct(imp_final, TOP_PCT_20)
    obj_top20_final = [c for c in top20_final if c in obj_cols_final]

    X_tr89_final = tf_final[top20_final].fillna(-999)
    X_test89_final = ttf_final[top20_final].fillna(-999)
    pseudo_mask_final = (best_sub < best_t) | (best_sub > 1 - best_t)
    pseudo_X_final = X_test89_final[pseudo_mask_final]
    pseudo_y_final = pd.Series((best_sub[pseudo_mask_final] > 0.5).astype(int), index=pseudo_X_final.index)
    X_aug_final = pd.concat([X_tr89_final, pseudo_X_final], axis=0)
    y_aug_final = pd.concat([y_final, pseudo_y_final], axis=0)
    m_final = _fit_one_classifier(X_aug_final, y_aug_final, obj_top20_final, OOF_SEED)
    test_pred_final = m_final.predict_proba(X_test89_final)[:, 1]

    np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_cb89_pseudo_best_oofpreds.npy", cb89_pseudo_oof[best_t])
    np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_cb89_pseudo_best_testpreds.npy", test_pred_final)
    logger.info("  OOF/Test予測(.npy)を保存しました(後日スタッキングへの組み込み候補)")
else:
    logger.info(f"[判定] 分解能未満(最良でも{best_diff:+.6f} <= {RESOLUTION})——"
                f"疑似ラベリングは不採用。CB89ベースライン(93_)を維持する")

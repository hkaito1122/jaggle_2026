"""115_stacking_with_lightgbm

`114_`で84_プール(134列)にLightGBM単体を適用した（early stopping込みで修正後、
単体val=0.549972、既存7モデルとの相関0.75〜0.93——cb144/tabpfn144/r_fullとは0.75〜0.81と
比較的低く、AutoGluonプールC(r_poolc51、109_で不採用確定)とは異なる、CatBoost/TabPFNとは
別のGBDT機構という軸での多様性を持つ）。107_の7モデルに8つ目として追加し、105_/107_/109_と
全く同一の設計（logit変換→L2ロジスティック回帰、外側5-fold(seed=12345)+内側
LogisticRegressionCVの二重入れ子）で再評価する。

`109_`（AutoGluonプールC追加）は入れ子CVでは改善(-0.0026、分解能未満)だったのにPublicでは
悪化(+0.0025)し方向が逆転した。この前例を踏まえ、本スクリプトでも分解能±0.0043を明確に
超えない限り上書き提出はしない。

出力: 分解能を超えて改善した場合のみ stacker8_lgbm_full_survivor_submission.csv
"""
import datetime
import sys
import warnings
...
SCRIPT_NAME = "115_stacking_with_lightgbm"
...
# ============================================================
# 保存済みOOF/Test予測の読み込み(107_の7モデル + 114_のlgbm134)
# ============================================================
...
# ============================================================
# スタッキング(入れ子CV) — 105_/107_/109_と全く同一設計、8モデルに拡張
# ============================================================
logger.info("=" * 60)
...
OUTER_SEED = 12345  # 105_/107_/109_と同一
...
if improvement_vs_equal > RESOLUTION and improvement_vs_stack7 > RESOLUTION:
    ...
    path = OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_stacker8_lgbm_full_survivor_submission.csv"

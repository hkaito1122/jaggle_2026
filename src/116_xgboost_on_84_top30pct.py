"""116_xgboost_on_84_top30pct

`114_`/`115_`でLightGBM単体を107_のスタックに8つ目として追加し、Public 0.493891の新記録
（107_比-0.000626）を達成した。option2で挙げていたもう一方の候補——XGBoostも同様の設計で
評価し、9つ目の候補としてスタックに追加できるか検証する。

`114_`の当初実装は`n_estimators=560`固定・early stoppingなしで深刻な過学習に陥り
（val=0.687〜0.697、predict_probaが0.0001〜0.9998まで極端に振れる）、修正後
（CatBoost相当の保守的な設定+early stopping）でval=0.549972まで回復した経緯がある。
本スクリプトは最初からこの教訓を反映し、early stopping込みで設計する。

`84_`のTOP30PCT_nested(447列中重要度上位30%=134列、86_/114_と同一の特徴量ソース・
同一fold割当)に対し、XGBoost単体をリークなしKFold OOFで評価する。カテゴリ列は
XGBoostのネイティブcategory dtypeサポート(enable_categorical=True)を使い、114_と同じ
Train+Test全体で確定させたcategoryコード表を使う（フレームごとに独立astype("category")
するとコードが食い違う致命的なバグを114_で発見済み、[[ensemble-oof-overfitting]]追記11
参照）。

列選択(重要度上位30%)は86_/114_と全く同じ設計でfoldごとに再計算する（選択リークを避ける、
[[validation-asymmetry]]のHow to apply参照）。

判定基準: 現最良115_(8モデル)にxgb134を9つ目として追加した入れ子CVスタッキングが、
分解能±0.0043を超えて改善するかで採否を決める（別スクリプトで実施）。本スクリプトは
OOF/Test予測の生成のみを担当する。

出力: xgb134_classifier（+ OOF・Test予測を.npyで保存し、後日スタッキングに使用）
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "116_xgboost_on_84_top30pct"
...
# ============================================================
# XGBoost統合: カテゴリ列はcategory dtypeに変換してネイティブ対応させる(enable_categorical=True)
# ============================================================
import xgboost as xgb
logger.info(f"xgboost version = {xgb.__version__}")

# 114_(LightGBM)の教訓を最初から反映: n_estimators固定・early stoppingなしは深刻な過学習
# （val=0.687〜0.697という壊滅的な結果）を招くと判明済みのため、CatBoostのA_PARAMS相当の
# 保守的な設定+fold学習データの一部をearly stopping用の内部検証に回す設計を最初から採用する。
XGB_PARAMS = dict(
...
XGB_INTERNAL_VAL_FRAC = 0.15  # fold学習データの一部をearly stopping用の内部検証に回す

# 全カテゴリ列のcategory dtypeコード表をTrain+Test全体から一度だけ確定させる
# （114_で発見した「フレームごとに独立astype("category")するとコードが食い違う」
# 致命的なバグの再発を防ぐ、[[ensemble-oof-overfitting]]追記11参照）。
...
def to_xgb_frame(df, feats, obj_cols):
    X = df[feats].copy()
    for c in obj_cols:
...
def fit_xgb(X_tr, y_tr, seed):
    ...
    n_val = max(1, int(n * XGB_INTERNAL_VAL_FRAC))
...
# ============================================================
# リークなしKFold OOF: 各foldでCatBoost選択(86_と同一)→XGBoostを評価
# ============================================================
logger.info("=" * 60)
logger.info("[XGBoost134] ネストされた特徴量選択(86_と同一)+XGBoost評価のOOFを構築中...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)
xgb_oof = np.zeros(len(train_ids))
ncols_per_fold = []
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    X_tr_xgb = to_xgb_frame(tf_f.loc[fold_train_ids], top30_cols, obj_top30)
    X_va_xgb = to_xgb_frame(tf_f.loc[fold_val_ids], top30_cols, obj_top30)
    m_xgb = fit_xgb(X_tr_xgb, y_tr_cls, OOF_SEED)
    xgb_oof[va_pos] = m_xgb.predict_proba(X_va_xgb)[:, 1]

    logger.info(f"    [fold{fold_i}] 完了 (列数={len(top30_cols)})")

xgb_score = log_loss(y_train.values[surv_mask], xgb_oof[surv_mask])
logger.info(f"[XGBoost134] val(OOF, リークなし, n={surv_mask.sum()})={xgb_score:.6f} "
            f"(fold毎列数: {ncols_per_fold})")
logger.info("(参考: 86_のCB134=0.515581 / TabPFN134=0.502742 / 114_のLightGBM134=0.549972。"
            "XGBoostが既存モデルとどの程度相関するかは後日スタッキングスクリプトで確認)")

# ============================================================
# OOFの保存(後日スタッキングに追加検討できるように)
# ============================================================
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_xgb134_oofpreds.npy", xgb_oof)
logger.info("  OOF予測(.npy)を保存しました")

# ============================================================
# 提出: Train全件で選択→XGBoost(5シード平均)でTest予測
# ============================================================
logger.info("=" * 60)
...
m_selector_full = _fit_selector(X_tr_full, y_tr_full, obj_cols_full, OOF_SEED)
...
logger.info(f"[提出用] TOP30PCT: {len(top30_cols_full)}列（86_/114_と同一の選択のはず）")

X_tr_xgb_full = to_xgb_frame(tf_full, top30_cols_full, obj_top30_full)
X_test_xgb_full = to_xgb_frame(ttf_full, top30_cols_full, obj_top30_full)

test_preds = []
for seed in SEEDS_SUB:
    m = fit_xgb(X_tr_xgb_full, y_tr_full, seed)
    test_preds.append(m.predict_proba(X_test_xgb_full)[:, 1])
    logger.info(f"    seed={seed}: 完了")
xgb_test = np.mean(test_preds, axis=0)
path1 = save_submission(xgb_test, "xgb134_classifier")
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_xgb134_testpreds.npy", xgb_test)

logger.info("=" * 60)
logger.info(f"局所OOF: XGBoost134={xgb_score:.6f}")

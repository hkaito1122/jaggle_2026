"""87_logistic_regression_on_77

`31_linear_model_rejected_features`/`32_ensemble_catboost_linear`（2026-08-11、当時のCatBoost
基準は`28_`のPublic 0.529454）で、L2正則化ロジスティック回帰を却下済み特徴量ブロックの
再検証に使ったことはあるが、それは441列よりずっと小さい当時の特徴量セット上での話だった。
以来パイプラインは444列→447列(77_)→134列(84_TOP30PCT_nested、Public 0.509500)へと
大きく進化しており、この規模の特徴量セットで線形モデルを組んだことは一度もない。

本スクリプトは、31_で確立した安定運用ずみの前処理レシピ
（欠損フラグ→中央値補完→標準化→[-10,10]クリップ、カテゴリはOne-Hot、
LogisticRegressionCVでCs=1e-4〜10を探索）を、現行の77_(447列)とその重要度選択版
84_TOP30PCT_nested(134列)の両方に適用し、リークなしKFold OOFで評価する
（選択は80_/81_/84_と同じfold毎の再計算パターン）。

CatBoost(同一447列/134列)を同じfoldで併せて学習し、ブレンド曲線(w=0〜1)のargminも
局所OOFで確認する（[[blend-curve-beats-val-margin-gate]]）。31_/32_時点では線形モデルの
絶対性能(val~0.57-0.59)がCatBoost比で弱すぎてブレンドが機能しなかった
（[[ensemble_oof_overfitting]]の「弱学習器税」）が、今回は特徴量セットが大幅に強化されて
いるため、その結論が今も成り立つかを確認する。

出力: logreg447_classifier / logreg134_classifier / (ブレンドが有望な場合のみ) blend_argmin
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "87_logistic_regression_on_77"
...
# ============================================================
# 線形モデル用前処理（31_linear_model_rejected_featuresの安定運用済みレシピをそのまま踏襲）
...
def prepare_linear_matrices(train_df, other_dfs, feat_cols):
    ...
    if obj_cols:
        ...
        cat_enc.append(ohe.fit_transform(tr_cat))
        for f in frames_raw[1:]:
...
# ============================================================
# リークなしKFold OOF: FULL447/TOP30PCT-134 それぞれで CatBoost と LogReg を比較
# ============================================================
logger.info("=" * 60)
logger.info("[LogReg447/LogReg134/CB447/CB134] リークなしKFold OOFを構築中...")
skf = StratifiedKFold(n_splits=OOF_N_SPLITS, shuffle=True, random_state=OOF_SEED)

cb447_oof = np.zeros(len(train_ids))
cb134_oof = np.zeros(len(train_ids))
logreg447_oof = np.zeros(len(train_ids))
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    X_va447 = tf_f.loc[fold_val_ids, feat_cols_f].fillna(-999)

    # CatBoost 447列(選択ステップ用の重要度モデルも兼ねる)
    m_cb447 = _fit_one_classifier(X_tr447, y_tr_cls, obj_cols_f, OOF_SEED)
    cb447_oof[va_pos] = m_cb447.predict_proba(X_va447)[:, 1]
    ...
    m_lr447 = fit_logreg(Xl_tr447, y_tr_cls.values)
    logreg447_oof[va_pos] = m_lr447.predict_proba(Xl_va447)[:, 1]

    # LogReg 134列(同じ134列選択を使う)
    ...
    logger.info(f"    [fold{fold_i}] 完了 (134列選択数={len(top30_cols)}, "
                f"LogReg447次元={Xl_tr447.shape[1]}, LogReg134次元={Xl_tr134.shape[1]})")

cb447_score = log_loss(y_train.values[surv_mask], cb447_oof[surv_mask])
cb134_score = log_loss(y_train.values[surv_mask], cb134_oof[surv_mask])
logreg447_score = log_loss(y_train.values[surv_mask], logreg447_oof[surv_mask])
logreg134_score = log_loss(y_train.values[surv_mask], logreg134_oof[surv_mask])

logger.info("=" * 60)
logger.info(f"[CB447]      val(OOF, n={surv_mask.sum()}) = {cb447_score:.6f}")
logger.info(f"[CB134]      val(OOF, n={surv_mask.sum()}) = {cb134_score:.6f}")
logger.info(f"[LogReg447]  val(OOF, n={surv_mask.sum()}) = {logreg447_score:.6f}  "
            f"(CB447比 {logreg447_score - cb447_score:+.6f})")
logger.info(f"[LogReg134]  val(OOF, n={surv_mask.sum()}) = {logreg134_score:.6f}  "
            f"(CB134比 {logreg134_score - cb134_score:+.6f})")
logger.info("(参考: 31_/32_時点のLogReg val~0.57-0.59、当時のCatBoost基準はPublic 0.529454。"
            "特徴量セットが444→447→134列へ進化した現在、絶対性能がどう変わったかを見る)")

# ============================================================
# ブレンド曲線(argmin)を局所OOFで確認: LogReg447×CB447 と LogReg134×CB134
# ============================================================
logger.info("=" * 60)
y_surv = y_train.values[surv_mask]


def _scan_blend(cb_oof, lr_oof, label):
    cb_s, lr_s = cb_oof[surv_mask], lr_oof[surv_mask]
    rows = []
    for w in BLEND_WEIGHTS:
        blend = np.clip(w * cb_s + (1 - w) * lr_s, 1e-9, 1 - 1e-9)
        ll = log_loss(y_surv, blend)
        rows.append((w, ll))
        logger.info(f"    [{label}] w_cb={w:.1f}: val={ll:.6f}")
    best_w, best_ll = min(rows, key=lambda r: r[1])
    corr = float(np.corrcoef(cb_s, lr_s)[0, 1])
    logger.info(f"[{label}] argmin: best_w_cb={best_w:.2f}, best_val={best_ll:.6f}, 相関={corr:.4f}")
    return best_w, best_ll, corr


logger.info("[ブレンド曲線] CB447 × LogReg447")
best_w447, best_ll447, corr447 = _scan_blend(cb447_oof, logreg447_oof, "447")
logger.info("[ブレンド曲線] CB134 × LogReg134")
best_w134, best_ll134, corr134 = _scan_blend(cb134_oof, logreg134_oof, "134")

# ============================================================
# 提出: Train全件でLogReg447/LogReg134を学習、必要ならブレンドも
# ============================================================
logger.info("=" * 60)
logger.info("提出用: Train全件でLogRegを学習中...")

m_selector_full = _fit_one_classifier(
    tf_full[FULL_FEATURE_COLS].fillna(-999), tf_full[TARGET_COL],
    [c for c in FULL_FEATURE_COLS if tf_full[c].dtype == "object"], OOF_SEED
)
...
m_lr447_full = fit_logreg(Xl_tr447_full, tf_full[TARGET_COL].values)
logreg447_test = m_lr447_full.predict_proba(Xl_test447_full)[:, 1]
path1 = save_submission(logreg447_test, "logreg447_classifier")

Xl_tr134_full, Xl_test134_full = prepare_linear_matrices(tf_full, [ttf_full], top30_cols_full)
m_lr134_full = fit_logreg(Xl_tr134_full, tf_full[TARGET_COL].values)
logreg134_test = m_lr134_full.predict_proba(Xl_test134_full)[:, 1]
path2 = save_submission(logreg134_test, "logreg134_classifier")

np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_logreg447_oofpreds.npy", logreg447_oof)
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_logreg134_oofpreds.npy", logreg134_oof)

logger.info("=" * 60)
logger.info("=== 全2ファイル出力完了(+ OOF保存) ===")
logger.info(f"  {path1}")
logger.info(f"  {path2}")

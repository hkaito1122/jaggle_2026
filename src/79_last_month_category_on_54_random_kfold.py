"""79_last_month_category_on_54_random_kfold

77_last_month_category_on_54 の再実装。77_は54_の`prepare_split`をそのまま踏襲した
「入社日で80/20に切る時系列ホールドアウト」を検証に使っていたが、[[validation_asymmetry]]
（2026-08-17決着）により、この分割方式の採用根拠（分布シフト）はEDA v7で否定済みであり、
535名(生存者)ホールドアウトの分解能は±0.0099しかないと判明している。77_の改善幅(-0.008525)は
この±0.0099を下回っており、判定不能の可能性がある。

本NBでは検証部分だけを「ランダムStratifiedKFold OOF（Train全2,761名、5-fold、単一seed）」に
差し替える（分解能±0.0043）。特徴量・Train全件学習・提出ファイルの中身は77_と完全に同一。

出力2つ:
  - baseline_444        : 54_ R0_memofix_plus_LM の忠実な再現（比較対象、この場限りのローカル基準、提出なし）
  - plus_lastmonth_cat   : baseline + 最終月の職種/勤務地/部署ID(3列, 447列)

val改善だけでは信用せず、必ずPublicで確認すること。
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "79_last_month_category_on_54_random_kfold"
...
# ============================================================
# NEW: 最終月の職種/勤務地/部署ID（xxxx_v4にはあるが54_に欠けていた生カテゴリ列）
#   xxxx_v4の月次集約関数と同一定義: 経過月数でソートした最終行(iloc[-1])の値をそのまま使う。
# ============================================================
...
logger.info("=" * 60)
logger.info("[NEW] 最終月の職種/勤務地/部署IDを生成中...")
...
NEW_COLS = ["最終月の職種", "最終月の勤務地", "最終月の部署ID"]
logger.info(f"  {NEW_COLS}")
for c in NEW_COLS:
    logger.info(f"  {c} ユニーク数: train={train_lastmonth_cat[c].nunique()}, test={test_lastmonth_cat[c].nunique()}")
...
# ============================================================
# prepare_split（54_と同一。add_lastmonthフラグで新規3列を足すかどうかを切り替える）
# ============================================================
def prepare_split_v77(split_ratio, add_lastmonth=True, exclude_early_from_val=True):
    ...
    # L2 + LM は54_のR0_memofix_plus_LMと同じく常に採用
...
def fit_oof_kfold_classifier(ag_full_, feature_cols, params, n_iter, seed=OOF_SEED, n_splits=OOF_N_SPLITS):
    ...
    77_の時系列80/20ホールドアウト(535名, 分解能±0.0099)の代わりに、Train全2,761名の
    ...
    for tr_idx, va_idx in skf.split(X, y):
        model = _fit_one_classifier(X.iloc[tr_idx], y.iloc[tr_idx], obj_cols, params, n_iter, seed)
        oof[va_idx] = model.predict_proba(X.iloc[va_idx])[:, 1]
    return {"val_seedavg": float(log_loss(y, oof))}
...
# ============================================================
# baseline_444: 54_ R0_memofix_plus_LM の忠実な再現（この場限りのローカル比較基準、提出はしない）
# ============================================================
logger.info("=" * 60)
logger.info("[baseline_444] 特徴量セットを構築中...")
ag_full_base, _, _ = prepare_split_v77(1.0, add_lastmonth=False, exclude_early_from_val=True)
BASE_FEATURE_COLS = _feature_cols(ag_full_base)
logger.info(f"[baseline_444] 特徴量数: {len(BASE_FEATURE_COLS)}（54_のR0_memofix_plus_LMは444のはず）")

logger.info("[baseline_444] 診断: ランダムStratifiedKFold OOF(Train全件, 5-fold)で検証中...")
hold_base = fit_oof_kfold_classifier(ag_full_base, BASE_FEATURE_COLS, A_PARAMS, ITER_HOLDOUT)
logger.info(f"[baseline_444] val_seedavg(OOF)={hold_base['val_seedavg']:.6f}（54_のR0_memofix_plus_LM Colab基準: 0.505477、分割方式が違うので単純比較不可）")


# ============================================================
# plus_lastmonth_cat: baseline_444 + 最終月の職種/勤務地/部署ID(3列)
# ============================================================
logger.info("=" * 60)
logger.info("[plus_lastmonth_cat] 特徴量セットを構築中...")
ag_full_plus, _, test_features_full_plus = prepare_split_v77(1.0, add_lastmonth=True, exclude_early_from_val=True)
PLUS_FEATURE_COLS = _feature_cols(ag_full_plus)
logger.info(f"[plus_lastmonth_cat] 特徴量数: {len(PLUS_FEATURE_COLS)}（444+3=447のはず）")
assert len(PLUS_FEATURE_COLS) == len(BASE_FEATURE_COLS) + 3, "新規3列以外の差異が発生している"

logger.info("[plus_lastmonth_cat] 診断: ランダムStratifiedKFold OOF(Train全件, 5-fold)で検証中...")
hold_plus = fit_oof_kfold_classifier(ag_full_plus, PLUS_FEATURE_COLS, A_PARAMS, ITER_HOLDOUT)
logger.info(f"[plus_lastmonth_cat] val_seedavg(OOF)={hold_plus['val_seedavg']:.6f}")

_delta = hold_plus["val_seedavg"] - hold_base["val_seedavg"]
logger.info("=" * 60)
logger.info(f"=== ローカル同一環境での比較(OOF, 分解能±0.0043): baseline_444={hold_base['val_seedavg']:.6f} → "
            f"plus_lastmonth_cat={hold_plus['val_seedavg']:.6f} (差={_delta:+.6f}) ===")
logger.info("val改善だけでは信用しない。Publicで確認するまでは採否を判断しない。")

logger.info("[plus_lastmonth_cat] Train全件学習 → Test予測(5シード平均)...")
test_preds_plus = fit_full_fixed_classifier(ag_full_plus, test_features_full_plus, PLUS_FEATURE_COLS, A_PARAMS, ITER_FULL, SEEDS_SUB)
pred_plus = test_preds_plus.mean(axis=0)
path_plus = save_submission(test_features_full_plus.index, pred_plus, "plus_lastmonth_cat")

logger.info("=" * 60)
logger.info("=== 実験完了 ===")
logger.info(f"  baseline_444 val_seedavg     = {hold_base['val_seedavg']:.6f}（提出なし、比較用）")
logger.info(f"  plus_lastmonth_cat val_seedavg = {hold_plus['val_seedavg']:.6f}（差 {_delta:+.6f}）")
logger.info(f"  提出ファイル: {path_plus}")

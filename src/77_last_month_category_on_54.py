"""77_last_month_category_on_54

xxxx_v4の重要度上位10に「最終月の職種」「最終月の勤務地」（54_には存在しない生カテゴリ列）が
入っていた。54_のcatchangeグループは変化回数/ユニーク数しか作っておらず、月次データの
「最終月時点の値そのもの」を一度も特徴量にしていない欠落だったため、これを54_の
R0_memofix_plus_LM(444列)にそのまま3列（最終月の職種・最終月の勤務地・最終月の部署ID）追加した
単一の事前登録済み構成として検証する。

出力2つ:
  - baseline_444        : 54_ R0_memofix_plus_LM の忠実な再現（比較対象、この場限りのローカル基準）
  - plus_lastmonth_cat   : baseline + 最終月の職種/勤務地/部署ID(3列, 447列)

val改善だけでは信用せず、必ずPublicで確認すること（このNBの結論は「提出候補になるか」までで、
採否はPublic確定後に判断する）。
"""
import datetime
import re
...
SCRIPT_NAME = "77_last_month_category_on_54"
...
def fit_holdout_fixed(ag_train, ag_val, feature_cols, params, n_iter, seeds):
    ...
    return {"val_seedavg": float(log_loss(y_va, val_preds.mean(axis=0)))}
...
ag_train_80_base, ag_val_surv_base, _ = prepare_split_v77(0.8, add_lastmonth=False, exclude_early_from_val=True)
BASE_FEATURE_COLS = _feature_cols(ag_train_80_base)
logger.info(f"[baseline_444] 特徴量数: {len(BASE_FEATURE_COLS)}（54_のR0_memofix_plus_LMは444のはず）")

logger.info("[baseline_444] 診断: 80/20ホールドアウト(生存者)で検証中(8シード平均)...")
hold_base = fit_holdout_fixed(ag_train_80_base, ag_val_surv_base, BASE_FEATURE_COLS, A_PARAMS, ITER_HOLDOUT, SEEDS_VAL)
logger.info(f"[baseline_444] val_seedavg={hold_base['val_seedavg']:.6f}（54_のR0_memofix_plus_LM Colab基準: 0.505477、ハードウェア差で完全一致はしない）")
...
ag_train_80_plus, ag_val_surv_plus, _ = prepare_split_v77(0.8, add_lastmonth=True, exclude_early_from_val=True)
...
logger.info("[plus_lastmonth_cat] 診断: 80/20ホールドアウト(生存者)で検証中(8シード平均)...")
hold_plus = fit_holdout_fixed(ag_train_80_plus, ag_val_surv_plus, PLUS_FEATURE_COLS, A_PARAMS, ITER_HOLDOUT, SEEDS_VAL)
logger.info(f"[plus_lastmonth_cat] val_seedavg={hold_plus['val_seedavg']:.6f}")

_delta = hold_plus["val_seedavg"] - hold_base["val_seedavg"]
logger.info("=" * 60)
logger.info(f"=== ローカル同一環境での比較: baseline_444={hold_base['val_seedavg']:.6f} → "

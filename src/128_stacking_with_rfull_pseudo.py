"""128_stacking_with_rfull_pseudo

`126_`で疑似ラベリングをR_full(72_reference_pipeline_standalone、115_の8モデルスタックで
全fold一貫して最大の係数(約+0.8)を持つ主力モデル)に適用した結果、単体では
**単調に**改善し続け(0.510980→0.479488、t=0.50=全2502件のTest行を疑似ラベルとして
追加、+0.031492)、123_のCB89のような山型ピークが一切現れなかった。この規模
(+0.0315)はこのプロジェクト全体を通じて最大の単発改善であり、pseudo版の予測は
元のR_fullよりも大幅に極端化している(Test予測std: 0.249→0.374、mean: 0.596→0.670)
——理論的な自己学習の峰型カーブと矛盾し、勝者の呪い([[ablation-cannot-settle-feature-blocks]])
というより「モデルが自分自身の予測を再学習して過信するようになった」循環的な
自己強化の懸念がある。123_→124_・125_→127_と同じ2段階検証フローの後半として、
115_の8モデルスタックのr_fullをrfull_pseudo(t=0.50版)に置換し、105_/107_/115_と
全く同一の設計でスタック全体の入れ子CVを再評価する。

**注意**: この規模の改善が仮にスタックレベルでも再現したとしても、単一seedでの
評価(72_オリジナルの3シード平均ではない)である点、Test予測が大幅に極端化している点
から、Public提出前に追加の検証(3シード平均での再計算等)を強く推奨する。

判定基準: 分解能±0.0043を正規にクリアした場合のみ採用する。ただし上記の懸念から、
分解能をクリアしても機械的に上書き提出はせず、必ずユーザーに追加検証の要否を確認する。

出力: 分解能を超えて改善した場合のみ stacker8_rfullpseudo_full_survivor_submission.csv
"""
import datetime
import sys
import warnings
...
SCRIPT_NAME = "128_stacking_with_rfull_pseudo"
...
# ============================================================
# 保存済みOOF/Test予測の読み込み(115_の8モデルのうちr_fullを126_のrfull_pseudoに置換)
# ============================================================
...
BASE_MODELS = {
    ...
    "rfull_pseudo": ("20260905/20260905_126_pseudolabel_rfull_poc_rfull_pseudo_best_oofpreds.npy",
                     "20260905/20260905_126_pseudolabel_rfull_poc_rfull_pseudo_best_testpreds.npy"),
...
rfull_orig_oof = np.load(OOF_DIR / "20260816/20260816_72_reference_pipeline_standalone_R_full_oofpreds.npy")
corr_pseudo_vs_orig = float(np.corrcoef(oof["rfull_pseudo"][surv_mask], rfull_orig_oof[surv_mask])[0, 1])
logger.info(f"[相関] rfull_pseudo vs 元のr_full(115_で使用): {corr_pseudo_vs_orig:.4f}")

# ============================================================
# スタッキング(入れ子CV) — 115_と全く同一設計、lgbm134をlgbm134_pseudoに置換して再評価
# ============================================================
logger.info("=" * 60)
logger.info("logit変換8モデルでのスタッキング(入れ子CV、r_full→rfull_pseudo置換版)")
...
logger.info(f"  [8モデルスタッキング(rfull_pseudo置換版、入れ子CV)] val={stack_val:.6f}")
...
if improvement_vs_equal > RESOLUTION and improvement_vs_stack7 > RESOLUTION:
    logger.info(f"[判定] 分解能を超えて改善(対等重み{improvement_vs_equal:+.6f} / 対元r_full版{improvement_vs_stack7:+.6f})"
    ...
    path = OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_stacker8_rfullpseudo_full_survivor_submission.csv"
    ...
    logger.info(f"[判定] 分解能未満(対等重み{improvement_vs_equal:+.6f} / 対元r_full版{improvement_vs_stack7:+.6f})"
                f"——rfull_pseudo置換版は不採用。判断はユーザーに委ねる")

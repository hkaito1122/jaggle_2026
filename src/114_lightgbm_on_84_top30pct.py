"""114_lightgbm_on_84_top30pct

`107_`のスタック(7モデル)はCatBoost系(cb134/cb89/cb144)とTabPFN系(tabpfn134/tabpfn89/
tabpfn144)とreference再現(r_full、CatBoostベース)のみで構成されており、実質2種類の
機構（CatBoost系・TabPFN系）に限られている。[[catboost-beats-other-gbdt]]でLightGBMは
単体でCatBoostに113列で+0.032劣ると分かっているが、これは「単体性能」の話であり、
TabPFN134が単体でCatBoost比+0.002未満まで肉薄しつつ相関0.94という高相関でもスタックに
寄与したように、**単体で劣っていても十分に異なる機構ならスタック内で寄与しうる**
（[[ensemble-oof-overfitting]]追記8: 「絶対性能差より相関の低さが効くケースがある」）。

`84_`のTOP30PCT_nested(447列中重要度上位30%=134列、86_のCB134/TabPFN134と同一の
特徴量ソース・同一fold割当)に対し、LightGBM単体をリークなしKFold OOFで評価する。
AutoGluonというラッパー経由ではなく、LightGBMという機構そのものを直接評価する点が
`102_`/`104_`（AutoGluon新特徴量統合、不採用確定）とは異なる——AutoGluonの内部の
複雑さ（zeroshotポートフォリオ選択・8-foldバギング・stacking）が悪さをしていた
可能性を排除し、機構そのものの価値を単体で見る。

列選択(重要度上位30%)は86_と全く同じ設計でfoldごとに再計算する（選択リークを避ける、
[[validation-asymmetry]]のHow to apply参照）。ハイパーパラメータは大まかな既定値
（Optuna探索はしない、単体性能の追求ではなく「スタックに追加する価値があるか」を見る
のが目的のため）。

判定基準: 107_の7モデルにlgbm134を8つ目として追加した入れ子CVスタッキングが、
分解能±0.0043を超えて改善するかで採否を決める（`105_`/`107_`と同一の判定フロー、
別スクリプトで実施）。本スクリプトはOOF/Test予測の生成のみを担当する。

出力: lgbm134_classifier（+ OOF・Test予測を.npyで保存し、後日スタッキングに使用）
"""
import datetime
import re
import sys
...
SCRIPT_NAME = "114_lightgbm_on_84_top30pct"
...
# ============================================================
# リークなしKFold OOF: 各foldでCatBoost選択(86_と同一)→LightGBMを評価
# ============================================================
logger.info("=" * 60)
logger.info("[LightGBM134] ネストされた特徴量選択(86_と同一)+LightGBM評価のOOFを構築中...")
...
for fold_i, (tr_pos, va_pos) in enumerate(skf.split(train_ids, y_train)):
    ...
    m_lgbm = fit_lgbm(X_tr_lgbm, y_tr_cls, obj_top30, OOF_SEED)
    lgbm_oof[va_pos] = m_lgbm.predict_proba(X_va_lgbm)[:, 1]

    logger.info(f"    [fold{fold_i}] 完了 (列数={len(top30_cols)})")

lgbm_score = log_loss(y_train.values[surv_mask], lgbm_oof[surv_mask])
logger.info(f"[LightGBM134] val(OOF, リークなし, n={surv_mask.sum()})={lgbm_score:.6f} "
            f"(fold毎列数: {ncols_per_fold})")
logger.info("(参考: 86_のCB134=0.515581 / TabPFN134=0.502742。"
            "LightGBMが両者とどの程度相関するかは後日スタッキングスクリプトで確認)")

# ============================================================
# OOFの保存(後日107_のスタッキングに追加検討できるように)
# ============================================================
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_lgbm134_oofpreds.npy", lgbm_oof)
logger.info("  OOF予測(.npy)を保存しました")

# ============================================================
# 提出: Train全件で選択→LightGBM(5シード平均)でTest予測
# ============================================================
logger.info("=" * 60)
...
logger.info(f"[提出用] TOP30PCT: {len(top30_cols_full)}列（86_と同一の選択のはず）")

X_tr_lgbm_full = to_lgbm_frame(tf_full, top30_cols_full, obj_top30_full)
X_test_lgbm_full = to_lgbm_frame(ttf_full, top30_cols_full, obj_top30_full)

test_preds = []
for seed in SEEDS_SUB:
    m = fit_lgbm(X_tr_lgbm_full, y_tr_full, obj_top30_full, seed)
    test_preds.append(m.predict_proba(X_test_lgbm_full)[:, 1])
    logger.info(f"    seed={seed}: 完了")
lgbm_test = np.mean(test_preds, axis=0)
path1 = save_submission(lgbm_test, "lgbm134_classifier")
np.save(OUTPUT_DIR / f"{TODAY}_{SCRIPT_NAME}_lgbm134_testpreds.npy", lgbm_test)

logger.info("=" * 60)
logger.info(f"局所OOF: LightGBM134={lgbm_score:.6f}")
